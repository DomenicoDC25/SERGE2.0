...
public class GameMechanics : MonoBehaviourPunCallbacks, IPunTurnManagerCallbacks, IOnEventCallback
{
    ...
    private bool isFirstTurnInRiskManagement = true; //determina se � il primo turno durante la fase di Risk Management
    ...
    void Update()
    {
        ...
        else if (gameState.Equals(GameState.CardReading) && isGameStarted && !finishedReading)
        {
            ...
            GameObject.Find("Fase (sottotesto)").GetComponent<TMP_Text>().text = "Cosa c'� scritto sulla carta?";
        }
        else if (gameState.Equals(GameState.CardReading) && isGameStarted && finishedReading)
        {
            GameObject.Find("Fase (testo)").GetComponent<TMP_Text>().text = "Card Reading";
        ...
        if (isLocalPlayerTurn && gameState.Equals(GameState.RiskManagement))
        {
            ...
            {
                GameObject.Find("Fase (sottotesto)").GetComponent<TMP_Text>().text = "� il tuo turno";
        ...
        if (move != null) 
        {
            if ((gameState.Equals(GameState.RiskManagement)))
            {
                ...
                else
                {
                    ...
                    Debug.LogError("� la fase di Risk Management");
                    ...
                    else if (actual_move[2] == 1) // carta rischio
                    {
                        if (!hasPlayersTurnCompletitionIncreased)
                        {
                            ...
                            Debug.LogError("Oh No! � una carta rischio");
                        }
                        eventsCount = 0;
                        Debug.LogError("� una carta rischio");
    ...
    void startNextTurn(Player lastplayer, bool isItAnewTurn) //Vedere se riesce a saltare da un giocatore all'ultimo
    {
        ...
        finally
        {
            if (lastplayer == null)
            {
                ...
                isLocalPlayerTurn = false; //blocca la possibilit� di lanciare il dado
                areCardsSelectable = false; //blocca la possibilit� di scegliere le carte a prescindere da quelle scelte dal compagno
    ...
    void IPunTurnManagerCallbacks.OnTurnCompleted(int turn)
    {
        ...
        if (gameState.Equals(GameState.RiskAnalysis))
        {
            ...
            else
            {
                ...
                areCardsSelectable = false;
                if (objectingVotes == 2) //Si � gi� stati nello stato di planning poker nel turno precedente
                {
                    Debug.LogError("Si � gi� stati nello stato di planning poker, passo a RiskAnalysis");
    ...
    void BeginMyTurn()
    {
        ...
        if (!ActorNumbers.Contains(PhotonNetwork.LocalPlayer.ActorNumber) || TeamSelection.isTeamMate) //Se � un compagno di squadra 
        {
            Debug.LogError("Il giocatore di turno non � presente tra gli ActorNumbers");
            isLocalPlayerTurn = false; //blocca la possibilit� di lanciare il dado
            areCardsSelectable = false; //blocca la possibilit� di scegliere le carte a prescindere da quelle scelte dal compagno
            if (gameState.Equals(GameState.PlanningPoker))
            {
    ...
    void spawnCard(bool IsEventCard, int index)
    {
        ...
        if (IsEventCard)
        {
            ...
            //Il nome delle carte nella lista � lo stesso dei prefab!
            Quaternion rotation = new Quaternion(0, PlayerController.playerCam.rotation.y, 0, 0);
    ...
    void spawnCardFromBoard(byte pile, int index)
    {
        ...
        if (actualCard == null)
        {
            ...
            //Mostra la probabilit� sulla carta
    ...
    //move[2] = 0 allora � una carta evento altrimenti
    //move[2] = 1 se � una carta rischio.
    ...
    int verifyRiskContingency(RiskCard r)
    {
        ...
        switch (r.impact)
        {
            case 0:
                ...
                {
                    if (UserGameSheet.resources[a] >= 0)//Per la risorsa richiesta � stato speso un token verde o uno con valore maggiore
                    {
                        points = 3;
                    }
                }
            ...
            case 1:
                ...
                {
                    if (UserGameSheet.resources[a] >= 1) //Per la risorsa richiesta � stato speso un token giallo o uno con valore maggiore
                    {
                        points = 3;
                    }
            ...
            case 2:
                ...
                {
                    if (UserGameSheet.resources[a] >= 2)//Per la risorsa richiesta � stato speso un token rosso
                    {
                        points = 3;
                    }
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        else if (code == printPlayerTurnEvent) //printPlayerTurn
        {
            ...
            if (fase.activeSelf)
            {
                GameObject.Find("Fase (sottotesto)").GetComponent<TMP_Text>().text = "� il turno di " + nickname;
                areCardsSelectable = false;
            }
        }
        ...
        else if (code == playerLeftEvent)
        {
            ...
            Logger.Instance.LogInfo($"<color=yellow>{nickname}</color> si � disconnesso dalla partita");
            LogManager.Instance.LogInfo($"{nickname} si � disconnesso dalla partita");
        }
    }
    ...
    void terminateGame()
    {
        ...
        if (true)
        {
            ...
            isLocalPlayerTurn = false; //blocca la possibilit� di lanciare il dado
            areCardsSelectable = false; //blocca la possibilit� di scegliere le carte a prescindere da quelle scelte dal compagno
            if (fase.activeSelf && (GameObject.Find("Fase (sottotesto)").GetComponent<TMP_Text>().text.Equals("� il tuo turno") || 
